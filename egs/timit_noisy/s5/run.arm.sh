...
function make_masked_fbank {
  ...
  local/make_masked_fbank.sh --cmd "$train_cmd" --nj $nj --mask-type arm \
    $src_dir $dest_dir $arm_nnet_dir $exp/make_masked_fbank/$y $arm_dir $fbankdir
...
while read noise_type <&3; do
  while read snr <&4; do
    ...
    # Make arm targets for each noise condition that would be used to
    # train the arm predictor neural net
    for x in train dev test; do 
      if [ ! -f data_fbank/${x}_noisy_${noise_type}_snr_$snr/.arm_targets.done ]; then
        local/make_arm_targets.sh --nj $nj --cmd "$train_cmd" data_fbank/${x}_noisy_${noise_type}_snr_$snr \
          data_fbank/${x}_clean $exp/make_arm_targets/${x}_noisy_${noise_type}_snr_$snr arm_targets || exit 1
        touch data_fbank/${x}_noisy_${noise_type}_snr_$snr/.arm_targets.done
      fi
    done
    ...
    if [ ! -f data_noisy_fbank/train_noisy_${noise_type}_snr_$snr/.arm_targets.done ]; then
      ...
      # The prefix is added to arm_targets.scp as well
      cat data_fbank/train_noisy_${noise_type}_snr_$snr/arm_targets.scp | \
        awk '{print "'${noise_type}'-snr-'$snr'-"$1" "$2}' | sort -k1,1 \
        > data_noisy_fbank/train_noisy_${noise_type}_snr_$snr/arm_targets.scp
      touch data_noisy_fbank/train_noisy_${noise_type}_snr_$snr/.arm_targets.done
    fi
...
if [ ! -f data_noisy_fbank/train_noisy/.done ]; then
  ...
  for x in `echo $noisy_dirs | tr ' ' '\n'`; do 
    [ -d $x ] && cat $x/arm_targets.scp
  done | sort -k1,1 > data_noisy_fbank/train_noisy/arm_targets.scp
...
if [ ! -f exp/arm_nnet/.done ]; then
  local/run_arm_nnet.sh --arm-scp data_noisy_fbank/train_noisy/arm_targets.scp \
    --datadir data_noisy_fbank/train_noisy
  touch exp/arm_nnet/.done
fi
...
# through the arm predictor neural net and applying the mask.
nj=$train_nj
if [ ! -f data_noisy_fbank/train_multi_masked/.done ]; then
  make_masked_fbank \
    data_noisy_fbank/train_multi \
    data_noisy_fbank/train_multi_masked exp/arm_nnet \
    arm masked_fbank
...
if [ ! -f data_fbank/train_clean_masked/.done ]; then
  make_masked_fbank \
    data_fbank/train_clean \
    data_fbank/train_clean_masked exp/arm_nnet \
    arm masked_fbank
  ...
  while read snr <&4; do
    for x in dev test; do
      ...
      if [ ! -f data_fbank/${x}_masked/.done ]; then
        make_masked_fbank data_fbank/${x} data_fbank/${x}_masked exp/arm_nnet arm masked_fbank
...
for x in dev test; do
  ...
  if [ ! -f data_fbank/${x}_masked/.done ]; then
    make_masked_fbank \
      data_fbank/${x} data_fbank/${x}_masked exp/arm_nnet arm masked_fbank
    touch data_fbank/${x}_masked/.done 
  fi
  if [ ! -f data/${x}_masked/.done ]; then
    make_fbank_mfcc data_fbank/${x}_masked data/${x}_masked fbank_mfcc
