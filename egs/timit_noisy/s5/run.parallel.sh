...
while ! $flag; do
  ...
  while read noise_type <&3; do
    while read snr <&4; do
      for x in train dev test; do 
        x=${x}_noisy_${noise_type}_snr_${snr}
        if [ ! -f data_fbank/$x/.fbank.done ] || [ ! -f data/$x/.mfcc.done ]; then
          flag=false
        fi
      done
    done 4< snr.list
...
if [ ! -f data_fbank/train_clean_masked/.done ]; then
  make_masked_fbank \
    ...
    data_fbank/train_clean_masked exp/irm_nnet \
    irm masked_fbank
  ...
  while read snr <&4; do
    for x in dev test; do
      ...
      if [ ! -f data_fbank/${x}_masked/.done ]; then
        make_masked_fbank data_fbank/${x} data_fbank/${x}_masked exp/irm_nnet irm masked_fbank
...
for x in dev test; do
  ...
  if [ ! -f data_fbank/${x}_masked/.done ]; then
    make_masked_fbank \
      data_fbank/${x} data_fbank/${x}_masked exp/irm_nnet irm masked_fbank
    touch data_fbank/${x}_masked/.done 
  fi
  if [ ! -f data/${x}_masked/.done ]; then
    make_fbank_mfcc data_fbank/${x}_masked data/${x}_masked fbank_mfcc
