...
while [ $x -lt $num_iters ]; do
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    if $distort_egs; then
      if [ ! -f $egs_dir/cmvn_global_stats ]; then
        echo "You have asked to distort the egs, but CMVN global stats has not
        been computed in $egs_dir/cmvn_global_stats"
        exit 1
      fi
      ...
      $cmd $parallel_opts JOB=1:$num_jobs_nnet $dir/log/train.$x.JOB.log \
        ...
        nnet-distort-egs --variance=$variance \
        --cmvn-stats=$egs_dir/cmvn_global_stats ark:- ark:- \| \
        nnet-train$train_suffix --minibatch-size=$this_minibatch_size --srand=$x \
        --raw=true $objf_opts "$nnet" ark:- $dir/$[$x+1].JOB.nnet || exit 1;
    else
      $cmd $parallel_opts JOB=1:$num_jobs_nnet $dir/log/train.$x.JOB.log \
        nnet-shuffle-egs --buffer-size=$shuffle_buffer_size --srand=$x \
        ark:$egs_dir/egs.JOB.$[$x%$iters_per_epoch].ark ark:- \| \
