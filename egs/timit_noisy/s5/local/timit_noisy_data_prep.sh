...
# or lower-cased
if [ ! -d $timit_dir/TRAIN -o ! -d $timit_dir/TEST ] && [ ! -d $timit_dir/train -o ! -d $timit_dir/test ]; then
  echo "timit_noisy_data_prep.sh: Spot check of command line argument failed"
...
if [ -d $timit_dir/TRAIN ]; then
  [ -d $timit_dir/train -o -d $timit_dir/test ] \
...
if $uppercased; then
  ...
  ls -d "$timit_dir"/TRAIN/DR*/* | sed -e "s:^.*/::" > $tmpdir/train_spk
else
...
for x in train dev test; do
  ...
  find $timit_dir/{$train_dir,$test_dir} -not \( -iname 'SA*' \) -iname '*.RAW' \
    | grep -f $tmpdir/${x}_spk > ${x}_sph.flist

  sed -e 's:.*/\(.*\)/\(.*\).RAW$:\1_\2:i' ${x}_sph.flist \
  ...
  find $timit_dir/{$train_dir,$test_dir} -not \( -iname 'SA*' \) -iname '*.PHN' \
