// nnet2bin/nnet-copy-multiview-egs.cc
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "\n"
        "Usage:  nnet-copy-multiview-egs [options] <egs-rspecifier> <egs-wspecifier1> [<egs-wspecifier2> ...]\n"
        "\n"
        "e.g.\n"
        "nnet-copy-multiview-egs ark:train.egs ark,t:text.egs\n"
        "or:\n"
        "nnet-copy-multiview-egs ark:train.egs ark:1.egs ark:2.egs\n";
    ...
    SequentialNnetMultiviewExampleReader example_reader(examples_rspecifier);

    int32 num_outputs = po.NumArgs() - 1;
    std::vector<NnetMultiviewExampleWriter*> example_writers(num_outputs);
    for (int32 i = 0; i < num_outputs; i++)
      example_writers[i] = new NnetMultiviewExampleWriter(po.GetArg(i+2));
      ...
      for (int32 c = 0; c < count; c++) {
        ...
        if (!copy_eg) {
          ...
          if (frame == -2)  // --frame=random was set -> choose random frame
            start_frame = RandInt(0, eg.views[0].labels.size() - 1);
          if (start_frame == -1 || start_frame < eg.views[0].labels.size()) {
            ...
            NnetMultiviewExample eg_mod(eg, start_frame, num_frames,
