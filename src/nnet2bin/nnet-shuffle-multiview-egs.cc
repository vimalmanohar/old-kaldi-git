// nnet2bin/nnet-shuffle-multiview-egs.cc
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "\n"
        "Usage:  nnet-shuffle-multiview-egs [options] <egs-rspecifier> <egs-wspecifier>\n"
        "\n"
        "nnet-shuffle-multiview-egs --srand=1 ark:train.egs ark:shuffled.egs\n";
    ...
    std::vector<std::pair<std::string, NnetMultiviewExample*> > egs;
    ...
    if (buffer_size == 0) { // Do full randomization
      ...
      for (; !example_reader.Done(); example_reader.Next())
        egs.push_back(std::make_pair(example_reader.Key(), 
                                    new NnetMultiviewExample(example_reader.Value())));
      
      std::random_shuffle(egs.begin(), egs.end());
    } else {
      KALDI_ASSERT(buffer_size > 0);
      egs.resize(buffer_size, std::pair<std::string, NnetMultiviewExample*>("", NULL));
      for (; !example_reader.Done(); example_reader.Next()) {
